...
- **OpenCode export wrote an invalid config when no models were discovered** - `config export --opencode` now fails with a clear error instead of emitting `"model": ""`.
- **OpenCode export hardcoded a placeholder `apiKey`** - The generated config is now pre-filled with the first configured client key (`security.api_keys`) so it works out of the box when auth is enabled; `-` is still written when no client auth is configured.
...
[Unreleased]: https://github.com/Wayazi/aperture-router/compare/v0.3.4...HEAD
[0.3.4]: https://github.com/Wayazi/aperture-router/compare/v0.3.3...v0.3.4
